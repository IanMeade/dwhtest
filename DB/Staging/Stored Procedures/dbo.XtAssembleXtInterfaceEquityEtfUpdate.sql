...
BEGIN    
	...
	EXEC dbo.XtAssembleXtInterfaceEquityEtfUpdateFixNulls
	...
	INSERT INTO    
		...
		SELECT   
			...
			IIF(Share.ExSpecial IS NOT NULL, IIF(Share.ExSpecial=0, 'N', 'Y' ),  D1.ExSpecial) AS ExSpecial, 
			IIF(Share.ExCapitalisation IS NOT NULL, IIF(Share.ExCapitalisation=0, 'N', 'Y' ),  D1.ExCapitalisation) AS ExCapitalisation, 
			IIF(Share.ExRights IS NOT NULL, IIF(Share.ExRights=0, 'N', 'Y' ),  D1.ExRights) AS ExRights, 
			IIF(Share.ExEntitlement IS NOT NULL, IIF(Share.ExEntitlement=0, 'N', 'Y' ),  D1.ExEntitlement) AS ExEntitlement, 
			IIF(Share.ExDividend IS NOT NULL, IIF(Share.ExDividend=0, 'N', 'Y' ),  D1.ExDividend) AS ExDividend, 
